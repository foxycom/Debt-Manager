...
JAVA_OPTS=" -Dabc.instrument.array.operations -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.chikeandroid.debtmanager "
...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.chikeandroid.debtmanager.develop$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.chikeandroid.debtmanager.develop.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall com.chikeandroid.debtmanager.develop > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall com.chikeandroid.debtmanager.develop.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" == "0" ]; then \
	...
	$(ADB) shell pm grant com.chikeandroid.debtmanager.develop android.permission.READ_CONTACTS;\
(>&2 echo "Installing test apk") ;\
	...
	$(ADB) shell pm clear com.chikeandroid.debtmanager.develop > /dev/null; \
	$(ADB) shell pm grant com.chikeandroid.debtmanager.develop android.permission.READ_CONTACTS;\
fi)
